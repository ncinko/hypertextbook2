...
// This monitor treats the incoming flux as a normalized power signal. It applies a detector lag
// and noise model, and displays counts per second on a log-like scale. At shutdown, counts
// hover around tens; at full power (flux ~ FULL_POWER_FLUX), counts reach the millions.
const FluxMonitor = ({ flux }) => {
  const [displayValue, setDisplayValue] = useState(0);
  const [history, setHistory] = useState(() => new Array(40).fill(0));
  const measRef = useRef(0);
  ...
  useEffect(() => {
    // Simulation constants for instrumentation
    const dt = 0.1;             // sample interval in seconds (100 ms)
    const TAU_DET = 0.8;        // detector response time constant (s)
    const a = 1 - Math.exp(-dt / Math.max(1e-6, TAU_DET));
    // Define the flux at which the plant produces 100% power.
    // In the core engine, flux typically hovers around 0.02 at nominal full power.
    const FULL_POWER_FLUX = 0.02;
    const CPS_SOURCE = 20;      // base counts per second in source range
    const CPS_FULL_POWER = 1e7; // counts per second at full power
    ...
    const interval = setInterval(() => {
      // Normalize power: clamp between 0 and ~120% of full power
      const powerNorm = Math.max(0, Math.min(1.2, fluxRef.current / FULL_POWER_FLUX));
      // Shape the response so low power grows slowly and high power steeply (quasi-log)
      const shaped = Math.pow(powerNorm, 1.7);
      const cpsTrue = CPS_SOURCE + shaped * (CPS_FULL_POWER - CPS_SOURCE);
      // Apply detector lag
      measRef.current = measRef.current + (cpsTrue - measRef.current) * a;
      // Noise: scaled to counts value plus extra for low counts
      const sigma = Math.max(2, 0.015 * measRef.current + 80 / Math.sqrt(measRef.current + 1));
      const cpsNoisy = Math.max(0, measRef.current + randn() * sigma);
      const cpsInt = Math.round(cpsNoisy);
      setDisplayValue(cpsInt);
  ...
  // Compress history onto a log-like scale for plotting
  const log10 = (x) => Math.log(x) / Math.log(10);
  const minPlot = 1;
  const logHist = history.map((v) => log10(Math.max(minPlot, v)));
  const logMax = Math.max(...logHist);
  const logMin = Math.min(...logHist);
  const span = Math.max(1e-6, logMax - logMin);
  const points = logHist.map((lv, i) => {
    const x = (i / (history.length - 1)) * 100;
    const y = 100 - ((lv - logMin) / span) * 90;
    return `${x},${y}`;
  }).join(" ");
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <div className="absolute top-1 right-1 text-green-400 text-[10px] font-mono bg-black/50 px-1 border border-green-900">
        {displayValue.toLocaleString()} CPS
      </div>
    </div>
  );
};
...
const CoreInternalsView = ({ state, onClose, onRodChange, onBoronChange }) => {
  // Recalculate reactivity in pcm for visualization - synchronized with the engine. The plant engine
  // uses pcm internally (1 pcm = 1e-5 Δk/k). Positive values add reactivity, negative values remove it.
  const clamp = (x, lo, hi) => Math.max(lo, Math.min(hi, x));
  const rodWithdrawFrac = clamp((100 - state.controlRodPosition) / 100, 0, 1);
  // Rods worth: up to +1500 pcm when fully withdrawn
  const RODS_MAX_PCM = 1500;
  const rho_rods_pcm = rodWithdrawFrac * RODS_MAX_PCM;
  // Boron worth: strong negative reactivity relative to reference concentration (1500 ppm)
  const BORON_REF_PPM = 1500;
  const BORON_WORTH_PCM_PER_PPM = 6.0;
  const rho_boron_pcm = -(state.boronConcentration - BORON_REF_PPM) * BORON_WORTH_PCM_PER_PPM;
  // Temperature feedback: negative reactivity proportional to deviation from 300°C
  const T_REF_C = 300;
  const TEMP_COEFF_PCM_PER_C = 4.0;
  const rho_temp_pcm = -(state.coreTemp - T_REF_C) * TEMP_COEFF_PCM_PER_C;
  // Use actual pressure if available from state; otherwise approximate from average temperature
  const avgTemp = (state.coolantTempHot + state.coolantTempCold) / 2;
  const pBar = typeof state.pressure === 'number' ? state.pressure : (1 + (avgTemp / 320) * 155);
  const satTemp = 100 + (pBar * 1.6);
  const subcoolMargin = satTemp - state.coolantTempHot;
  // If the hot leg is within 10°C of saturation, impose a strong negative penalty per degree margin shortfall
  const rho_subcool_pcm = subcoolMargin < 10 ? -(10 - subcoolMargin) * 120 : 0;
  // Offset ensures default conditions hover around critical (tune as needed)
  const RHO_OFFSET_PCM = -250;
  // Sum contributions
  let rho_pcm = rho_rods_pcm + rho_boron_pcm + rho_temp_pcm + rho_subcool_pcm + RHO_OFFSET_PCM;
  // If scrammed, force deeply negative reactivity
  if (state.isScrammed) rho_pcm = -6000;
          ...
          <div className="w-40 flex flex-col gap-2 min-h-0">
            <fieldset className="border border-white border-l-gray-500 border-t-gray-500 p-2 shrink-0">
              ...
              <Win95Inset className="h-32 bg-black relative flex items-end overflow-hidden">
                 {/* FluxMonitor now only takes flux; instrumentation handles noise and lag internally */}
                 <FluxMonitor flux={state.flux} />
              </Win95Inset>
            </fieldset>
...
};"text-red-600 font-bold" : reactivity < -0.001 ? "text-blue-600" : "text-gray-600"}>
...
const PWRSimulator = () => {
  ...
  const updatePhysics = (deltaTime) => {
    setSimState(prev => {
      ...
      let newTurbineRPM = prev.turbineRPM

      const drive = 0.01 * steamFlowRate;           // torque-ish
      const drag  = 0.01  * prev.turbineRPM**2;  // aerodynamic/generator losses
      newTurbineRPM += (drive - drag) * dt;
      newTurbineRPM = Math.min(newTurbineRPM, 6000);
