    ...
    >
      ...
      >
        ...
        <div className="pel-panel" style={{ padding: 12, height: sharedH, display: "flex", flexDirection: "column", margin: 0 }}>
          <h4 style={{ margin: "4px 0 8px", color: "#111" }}>Energy</h4>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, alignItems: "end", flex: 1, border: "1px solid #e5e7eb", borderRadius: 10, padding: 10 }}>
...
function EnergyReadout({ mode, qRef, vRef, bars }) {
  const number = (x) => x.toFixed(3).replace(/\.000$/, ".0");
  return (
    <div style={{ marginTop: 10, fontSize: 13, lineHeight: 1.45, color: "#374151" }}>
...
function Bar({ label, color, fillRef }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", height: "100%" }}>
      <div style={{ flex: 1, width: "80%", background: "#f3f4f6", borderRadius: 10, position: "relative", boxShadow: "inset 0 1px 3px rgba(0,0,0,0.06)" }}>
        <div ref={fillRef} style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "0%", background: color, borderRadius: 10 }} />
      </div>
      <div style={{ marginTop: 6, fontWeight: 700, color: "#1f2937" }}>{label}</div>
    </div>
  );
}
