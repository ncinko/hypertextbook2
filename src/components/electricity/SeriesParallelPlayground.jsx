...
export default function SeriesParallelPlayground({ width = 940, height = 580 }) {
  ...
  const RBeff = SB ? (RB1 + RB2) : Infinity;
  const hasAny = isFinite(RAeff) || isFinite(RBeff);
  const Rpar = hasAny ? 1 / ([RAeff, RBeff].filter(isFinite).reduce((s, r) => s + 1 / r, 0)) : Infinity;
  const Rtot = hasAny ? Rb + Rpar : Infinity;
  ...
  return (
    <div style={{ width, margin: "0 auto", color: "#e8f1ff", fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      {/* Controls */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 12, background: "#0e1a28", padding: 12, borderRadius: 12, border: "1px solid rgba(147,197,253,0.2)", marginTop: 10 }}>
      ...
      <p style={{ marginTop: 8, fontSize: 13, opacity: 0.8 }}>
        Per-branch vertical feeds now only animate when their branch carries current; the split spine remains visible without motion. Battery icon is relabeled (+/−) and repositioned.
      </p>
    </div>
  );
}
