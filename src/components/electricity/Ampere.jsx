// src/components/StraightCurrentFieldAmpere.jsx
...
export default function StraightCurrentFieldAmpere() {
  ...
  const pathStepsRef = useRef([]); // array of steps with segment + B·dℓ data
  const frameCounterRef = useRef(0); // for slowing animation
  ...
  useEffect(() => {
    ...
    function drawArrow(baseX, baseY, vx, vy, strength, highlighted) {
      ...
      let headSize;
      if (highlighted) {
        // Strong highlight: pale yellow, bigger
        L = maxLen + 10;
        strokeStyle = "rgba(252, 211, 77, 0.95)"; // pale yellow
        lineWidth = 3.4;
        headSize = 11;
      } else {
        ...
        const hue = 215;
        const lightness = 55;
        strokeStyle = `hsla(${hue}, 80%, ${lightness}%, ${alpha})`;
        lineWidth = 1.7;
        headSize = 7;
      }
    ...
    if (pathPoints.length > 1) {
      ctx.strokeStyle = integrating ? "#22c55e" : "#e5e7eb";
      ctx.lineWidth = 2;
      ctx.beginPath();
    ...
    if (integrating || integrationIndex >= 0) {
      ...
      if (
        ...
        ctx.strokeStyle = "rgba(252, 211, 77, 0.95)"; // pale yellow
        ctx.lineWidth = 4;
        ctx.beginPath();
  ...
  // Build integration steps (now using *direct* B at each midpoint for accuracy)
  function buildPathSteps() {
    ...
    const steps = [];
    let cumulative = 0;
    const stepSizePx = 12; // path step resolution along the curve
    ...
    for (let i = 0; i < pts.length - 1; i++) {
      ...
      for (let k = 0; k < nSteps; k++) {
        ...
        // Find nearest *visual* grid vector for highlighting
        let bestIndex = 0;
        ...
        // But compute B *directly* at (sx, sy) for higher accuracy
  ...
  function startIntegration() {
    ...
    const framesPerStep = 2; // >1 slows the animation a bit
  ...
  return (
    <div
      className="mx-auto my-4 p-4 rounded-xl shadow-lg"
      style={{
        width: "100%",
        background:
      ...
      >
        ...
        <div style={{ flex: "0 0 260px", fontSize: "0.9rem" }}>
          ...
          >
            ...
            >
              Click on the canvas to place currents in the selected mode. Drag in{" "}
              <span style={{ fontWeight: 600 }}>Move</span> mode to reposition.
            </div>
            <div
              style={{ marginTop: "0.35rem", fontSize: "0.8rem", color: "#e5e7eb" }}
            >
          ...
          >
            ...
            <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
              ...
              >
                {integrating ? "Integrating…" : "Integrate ∫B·dℓ"}
              </button>
              <button
                type="button"
                onClick={resetPath}
          ...
          >
            ...
            >
              <li style={{ marginBottom: "0.2rem" }}>
                ⨀ (dot): current out of the screen. Curl your fingers to see{" "}
                <b>B</b> direction (counterclockwise).
              </li>
              <li style={{ marginBottom: "0.2rem" }}>
                ⨂ (cross): current into the screen. Field direction reverses
                (clockwise).
              </li>
