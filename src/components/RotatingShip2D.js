...
export default function RotatingShip2D() {
  ...
  function handleCanvasDown(e) {
    ...
    state.current.x = x0; state.current.y = y0;
    state.current.vx = 0; state.current.vy = 0; state.current.placed = true;
  ...
  function handleCanvasMove(e) {
    ...
    state.current.vx = vx; state.current.vy = vy;
    drag.current.hasVelocity = mag(vx, vy) > 1e-3;
  }
  function handleCanvasUp() { drag.current.dragging = false; }
...
useEffect(() => {
  ...
  // ---------- drawing helpers ----------
  function drawArrow(ctx, x0, y0, x1, y1, color = "#08c", width = 2, head = 10) {
    ...
    ctx.beginPath();
    ctx.moveTo(sx1, sy1);
    ctx.lineTo(sx1 - head * Math.cos(ang - Math.PI / 7), sy1 - head * Math.sin(ang - Math.PI / 7));
    ctx.lineTo(sx1 - head * Math.cos(ang + Math.PI / 7), sy1 - head * Math.sin(ang + Math.PI / 7));
  ...
  function drawStars(ctx, theta) {
    ...
    for (const st of ship.current.stars) {
      ...
      ctx.fillStyle = "#fff";
      ctx.beginPath(); ctx.arc(sx, sy, st.s, 0, 2 * Math.PI); ctx.fill();
    }
    ctx.globalAlpha = 1;
  }
  ...
  function drawShip(ctx, theta) {
    ...
    if (showFloor) {
      ...
      ctx.beginPath(); ctx.arc(0, 0, ship.current.R, 0, 2 * Math.PI);
      ctx.fill();
    }
    ctx.beginPath(); ctx.arc(0, 0, ship.current.R, 0, 2 * Math.PI); ctx.stroke();
    ...
    if (showFloor && showGrid) {
      ...
      for (let i = 0; i < 24; i++) {
        ...
        ctx.beginPath();
        ctx.moveTo(0, 0);
        ctx.lineTo(ship.current.R * Math.cos(a), ship.current.R * Math.sin(a));
        ctx.stroke();
      }
      // rings each R/6
      for (let k = 1; k <= 5; k++) {
    ...
    for (let i = 0; i < 12; i++) {
      const a = (i * Math.PI) / 6;
      ctx.beginPath();
      ctx.moveTo((ship.current.R - 14) * Math.cos(a), (ship.current.R - 14) * Math.sin(a));
      ctx.lineTo((ship.current.R - 2) * Math.cos(a), (ship.current.R - 2) * Math.sin(a));
      ctx.stroke();
    }
    ctx.fillStyle = "rgba(120,180,255,0.9)";
    ctx.beginPath(); ctx.arc(0, +ship.current.R * 0.55, 12, 0, 2 * Math.PI); ctx.fill();
  ...
  function drawHUD(ctx) {
    ...
    ctx.fillStyle = "#fff";
    ctx.font = `${14 * dpiScale}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto`;
    ctx.fillText(`Frame: ${frame === "star" ? "Star (inertial)" : "Ship (rotating)"}`, 10 * dpiScale, 22 * dpiScale);
    const rpm = (omega / (2 * Math.PI)) * 60;
    ctx.fillText(`ω = ${omega.toFixed(2)} rad/s  (${rpm.toFixed(2)} rpm)`, 10 * dpiScale, 42 * dpiScale);
    ...
    if (state.current.placed) {
      ...
      const [rx, ry] = rot(state.current.x, state.current.y, -theta);
      const ship_vx = -omega * ry, ship_vy = omega * rx;
      const v_rel_x = vIxr - ship_vx, v_rel_y = vIyr - ship_vy;
      const vdisp = frame === "ship" ? mag(v_rel_x, v_rel_y) : mag(state.current.vx, state.current.vy);
      ctx.fillText(`|v| = ${vdisp.toFixed(2)} (units/s)`, 10 * dpiScale, 62 * dpiScale);
    }
    ctx.restore();
  }
  ...
  function draw(dt) {
    ...
    const ship_vx = -omega * ry, ship_vy = omega * rx;   // Ω×r in ship axes
    ...
    if (s.placed) {
      ...
      if (showVelocity) {
        const [dvx, dvy] = frame === "ship" ? [v_rel_x, v_rel_y] : [s.vx, s.vy];
        drawArrow(ctx, wx, wy, wx + dvx * 0.25, wy + dvy * 0.25, "#10b981", 3, 12);
      }
...
// -2 Ω×v_rel
        ...
        const scale = 0.25;
        if (showCoriolis) drawArrow(ctx, rx, ry, rx + cor[0] * scale, ry + cor[1] * scale, "#ef4444", 3, 12);
        if (showCentrifugal) drawArrow(ctx, rx, ry, rx + cen[0] * scale, ry + cen[1] * scale, "#3b82f6", 3, 12);
      }
      ...
      ctx.fillStyle = "#fff";
      ctx.beginPath(); ctx.arc(sx, sy, 5 * dpiScale, 0, 2 * Math.PI); ctx.fill();
    ...
    if (drag.current.dragging && state.current.placed) {
      ...
      const [sx0, sy0] = frame === "ship" ? rot(start.x, start.y, -theta) : [start.x, start.y];
      const [sx1, sy1] = frame === "ship" ? rot(cur.x, cur.y, -theta) : [cur.x, cur.y];
  ...
  }, [omega, frame, running, dpiScale, size.w, size.h, showCoriolis, showCentrifugal, showVelocity, showTrail, showFloor, showGrid]);

  // ---------- reset ----------
  function reset() {
    state.current = { x: 0, y: 0, vx: 0, vy: 0, placed: false };
    drag.current = { dragging: false, startWorld: null, currentWorld: null, hasVelocity: false };
    trailRef.current = [];
    tRef.current = 0;
  }
  ...
  return (
    <div
      ref={containerRef}
      style={{ width: "100%", display: "grid", gridTemplateColumns: "1fr", placeItems: "center", gap: 12, padding: 12 }}
    >
      <div style={{ width: "min(95vw,1200px)", display: "grid", gridTemplateColumns: "1fr", gap: 10 }}>
        {/* Controls */}
        <div
          style={{
            display: "grid",
        ...
        >
          <Card title="Reference Frame">
            <Row>
              <Btn active={frame === "star"} onClick={() => setFrame("star")}>Star (inertial)</Btn>
              <Btn active={frame === "ship"} onClick={() => setFrame("ship")}>Ship (rotating)</Btn>
            </Row>
                      <Row><Btn onClick={dropFromShipFrame}>Drop ball (ship frame)</Btn></Row>
          </Card>
          ...
          <Card title="Angular Speed ω (rad/s)">
            ...
            <div style={{ marginTop: 6, fontSize: 13, opacity: 0.9 }}>
              Current: <b>{omega.toFixed(2)}</b> rad/s &nbsp;|&nbsp; RPM: <b>{((omega / (2 * Math.PI)) * 60).toFixed(2)}</b>
            </div>
          </Card>

          <Card title="Overlays">
            <Check label="Velocity vector" checked={showVelocity} onChange={setShowVelocity} />
            <Check label="Coriolis acceleration (ship frame)" checked={showCoriolis} onChange={setShowCoriolis} disabled={frame !== "ship"} dim={frame !== "ship"} />
            <Check label="Centrifugal acceleration (ship frame)" checked={showCentrifugal} onChange={setShowCentrifugal} disabled={frame !== "ship"} dim={frame !== "ship"} />
            <Check label="Trail" checked={showTrail} onChange={setShowTrail} />
            <Check label="Deck grid" checked={showGrid} onChange={setShowGrid} disabled={!showFloor} dim={!showFloor} />
          </Card>

          <Card title="Simulation">
            <Row>
              <Btn active={running} onClick={() => setRunning(r => !r)} aria-pressed={running}>
                {running ? "Pause" : "Play"}
              </Btn>
              <Btn onClick={reset}>Reset</Btn>
            </Row>
            <div style={{ fontSize: 12, opacity: 0.9, marginTop: 8 }}>
              Click inside the ship to place the object, then drag to set its initial velocity.
            </div>
          </Card>
        </div>
        ...
        <div
          style={{
            display: "grid",
            placeItems: "center",
            background: "radial-gradient(1000px 600px at 50% 50%, #0a1020, #050811 70%, #02050b 100%)",
            borderRadius: 16,
            padding: 8,
...
function Check({ label, checked, onChange, disabled, dim }) {
  return (
    <label style={{ display: "flex", alignItems: "center", gap: 6, margin: "2px 0", fontSize: 14 }}>
      <input type="checkbox" checked={checked} onChange={e => onChange(e.target.checked)} disabled={disabled} />
      <span style={{ marginLeft: 6, color: dim ? "#9ca3af" : "#fff" }}>{label}</span>
    </label>
  );
}
