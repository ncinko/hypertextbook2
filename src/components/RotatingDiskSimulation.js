...
const RotatingDiskSimulation = () => {
  ...
  useEffect(() => {
    const animate = (time) => {
      if (previousTimeRef.current !== undefined) {
        ...
        const I = 0.5 * mass * Math.pow(radius, 2);
  ...
  return (
    <div>
      <h3>Rotating Disk Simulation</h3>
      <div style={{ textAlign: "center" }}>
        <div style={diskStyle}></div>
        <p>Angle: {angle.toFixed(2)} rad</p>
        <p>Angular Velocity: {angularVelocity.toFixed(2)} rad/s</p>
        <p>Moment of Inertia: {(0.5 * mass * Math.pow(radius, 2)).toFixed(2)} kg·m²</p>
        <p>
          Angular Acceleration:{" "}
          {(0.5 * mass * Math.pow(radius, 2) !== 0
            ? (torque / (0.5 * mass * Math.pow(radius, 2))).toFixed(2)
            : "0")}{" "}
          rad/s²
        </p>
      </div>
...
export default RotatingDiskSimulation;
